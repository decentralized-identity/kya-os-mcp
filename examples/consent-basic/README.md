...
| `scripts/generate-identity.ts` | Persist a DID to `.kya/identity.json` so it survives restarts |
...
This writes a `did:key` identity to `.kya/identity.json`. The server loads it automatically on startup.
