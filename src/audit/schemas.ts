...
const didSchema = z.string().startsWith('did:').min(5);
...
const detailsSchemas = [
  ...
  z.object({
    ...
    idempotencyRef: z.string().min(1).optional(),
  }).strict(),
  z.object({
  ...
  z.object({
    ...
    statusEvidence: z.array(evidenceRefSchema).optional(),
  }).strict(),
  z.object({
  ...
  z.object({
    ...
    previousEpochId: z.string().min(1).optional(),
    previousTerminalCheckpointDigest: digestSchema.optional(),
    successorEpochIds: z.array(z.string().min(1)).optional(),
...
const authorizationEvidenceSchema = z.object({
  ...
  delegationRef: z.string().min(1).optional(),
  delegationCredentialDigest: digestSchema.optional(),
  delegationChainDigests: z.array(digestSchema).optional(),
  statusEvidence: z.array(evidenceRefSchema).optional(),
  grantRef: z.string().min(1).optional(),
...
export const auditProducerEventSchema = z.object({
  ...
  binding: z.string().regex(/^urn:kya-os:audit-binding:[a-z0-9][a-z0-9._:-]*$/),
...
const auditBundleSelectionSchema = z.object({
  ...
  checkpointTreeSizes: z.array(decimalStringSchema),
...
export const auditBundleManifestCoreSchema = z.object({
  ...
  inventory: z.array(auditBundleInventoryItemSchema),
...
export const auditVerificationPolicySchema = z.object({
  ...
  acceptedIntegritySuites: z.array(z.string().min(1)).min(1),
...
export function parseAuditProducerEvent(input: unknown): AuditProducerEventCoreV1 {
  return deepFreeze(auditProducerEventSchema.parse(input) as AuditProducerEventCoreV1);
}

export function parseAuditEntryCore(input: unknown): AuditEntryCoreV1 {
  return deepFreeze(auditEntryCoreSchema.parse(input) as AuditEntryCoreV1);
}

export function parseSignedAuditEntry(input: unknown): SignedAuditEntryV1 {
  return deepFreeze(signedAuditEntrySchema.parse(input) as SignedAuditEntryV1);
}

export function parseAuditRecorderReceiptCore(input: unknown): AuditRecorderReceiptCoreV1 {
  return deepFreeze(auditRecorderReceiptCoreSchema.parse(input) as AuditRecorderReceiptCoreV1);
}

export function parseAuditCheckpointCore(input: unknown): AuditCheckpointCoreV1 {
  return deepFreeze(auditCheckpointCoreSchema.parse(input) as AuditCheckpointCoreV1);
}

export function parseSignedAuditCheckpoint(input: unknown): SignedAuditCheckpointV1 {
  return deepFreeze(signedAuditCheckpointSchema.parse(input) as SignedAuditCheckpointV1);
}
...
export function parseAuditReplayBundle(input: unknown): AuditReplayBundleV1 {
  return deepFreeze(auditReplayBundleSchema.parse(input) as AuditReplayBundleV1);
}
