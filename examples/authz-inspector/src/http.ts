 ...
 *
 * `createHttpServer(port)` returns the (not-yet-listening) server so callers —
 * including the e2e test — can drive it in-process; running this file directly
 * starts it listening on PORT.
 */
...
const DEFAULT_PORT = parseInt(process.env['PORT'] ?? '3030', 10);
...
/**
 * Build the demo's Streamable HTTP server: the consent page + authorization
 * server (/authorize, /approve, /callback, /token) and the stateful /mcp
 * transport. Returned not-yet-listening so the caller owns the port lifecycle.
 */
export function createHttpServer(port: number = DEFAULT_PORT): http.Server {
...
// Run directly (e.g. `npx tsx src/http.ts` / `npm run start:http`) → start listening.
const isMain = !!process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href;
if (isMain) {
  createHttpServer(DEFAULT_PORT).listen(DEFAULT_PORT, () => {
    process.stderr.write(`[kya-os] authz Inspector demo (Streamable HTTP) on http://localhost:${DEFAULT_PORT}/mcp\n`);
    process.stderr.write(`[kya-os] authorization server (consent page) on http://localhost:${DEFAULT_PORT}/authorize\n`);
  });
}
