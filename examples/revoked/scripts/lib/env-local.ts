 ...
 *
 * Anchored to the EXAMPLE root (resolved from this module), not process.cwd():
 * the server issues spare delegations at runtime and may have been spawned
...
const ENV_LOCAL = path.resolve(
  path.dirname(fileURLToPath(import.meta.url)), '..', '..', '.env.local',
);
