...
One distinction is owed precision: the KYA-OS Protocol Specification v1, including the Entity Card profile, was ratified as a DIF standard by the DIF Trusted AI Agents Working Group (TAAWG); the SEP's claims rest on the published documents and shipped implementations.
...
- **[Conformance vectors](https://github.com/decentralized-identity/kya-os-mcp/tree/main/conformance/vectors)**: a 48-vector machine-readable suite (suite 1.1.0, hash-pinned in `conformance/SUITE-MANIFEST.json`), including 8 negotiation vectors covering declared, absent, malformed, empty-object, initialize-era, and discovery-exemption cases against optional and required servers, with the [stdlib Python verifier](https://github.com/decentralized-identity/kya-os-mcp/blob/main/conformance/verify.py) as the second implementation for the proof and audit vector categories.
