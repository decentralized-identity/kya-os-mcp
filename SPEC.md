...
### 12.5 One-Off Delegation Keypairs

> *Non-normative.* This section describes a privacy pattern; it does not add normative requirements.

A delegator can issue a `DelegationCredential` to a one-off public key generated by the delegate solely for that delegation. The delegate's primary DID never appears as the credential subject; verifiers see only the ephemeral `did:key`. Because the key is not reused, the delegate's activity across different delegators and verifiers remains unlinkable.
