...
interface ProofMeta {
  ...
  prf?: 'org.kya-os/response-proof.envelope';
                         // Response-proof profile discriminator, COVERED by the
                         // JWS signature. ABSENT ⇒ the body profile (body-only
...
- **body** (`prf` ABSENT — the original profile): the `data` field only (excludes `_meta`).
  Under MCP carriage, `data` is the result's `content` array; result members outside it are NOT covered.
- **envelope** (`prf: "org.kya-os/response-proof.envelope"`): the ENTIRE result object with the top-level `_meta` member removed, mirroring the request side's `{method, params minus _meta}` rule.
  This authenticates result members such as `structuredContent`, `isError`, and `resultType` that the body profile leaves uncovered, while `_meta` stays intermediary-mutable — which is what lets the proof itself be attached there after signing.
...
Because `prf` is covered by the signature, stripping it from an envelope-profile proof breaks verification — a downgrade to body-only semantics is a hard failure, not a silent fallback.
Producers default to the body profile (wire compatibility with existing verifiers); the envelope profile is an explicit opt-in (`responseProofProfile` in the middleware configuration).
...
An envelope-profile proof (§7.3) additionally carries its `prf` claim in this payload — profile selection is signed, never advisory.
...
material only — the `data` body under the body profile, the result envelope minus
its top-level `_meta` under the envelope profile — and never covers `_meta` itself (§7.3).
